...
public class SwaggerConfig {
    @Value("${app.baseHost}")
    String serverUrl = "https://nicu-dev.cmedhealth.com";
                        ...
                        .addSecuritySchemes("oauth2-client-credentials", new SecurityScheme()
                                ...
                                .flows(new OAuthFlows()
                                        .clientCredentials(new OAuthFlow()
                                                .tokenUrl("/api/v1/oauth/token")
                                                .scopes(new Scopes()
                                                        .addString("m2m.read", "Machine read scope")
                                                        .addString("m2m.write", "Machine write scope")))))
        ...
        return openApi -> openApi.getPaths().values().forEach(pathItem ->
                pathItem.readOperations().forEach(operation ->
                        operation.addSecurityItem(new SecurityRequirement().addList("oauth2-password")
                                .addList("oauth2-client-credentials")
                                .addList("bearer-jwt"))));
    }
